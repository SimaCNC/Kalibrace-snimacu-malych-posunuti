...
#KALIBRACE KONKRETNIHO SNIMACE
#SMYCKA VLAKNO FUNKCE kalibrace s PULZY DOPREDNA
    """
    SMYCKA PRO PULZY DOPREDNA
    """            
    def kalibrace_start_pulzy_dopredna_kapacitni(self):
        print(f"[{self.__class__.__name__}] [{inspect.currentframe().f_code.co_name}] SMYCKA VLAKNO FUNKCE!!!")
        #self.controller.M_C_Index()
        self.controller.blok_widgets(self.controller.root) #zablokovani widgetu - M_C_Index neco odblokuje
        #pracovni slozka:
        ...
        def kalibrace_start_inner():
            ...
            print(f"[{self.__class__.__name__}] VLAKNO KALIBRACE!")
            self.pocet_kroku = math.floor(self.merena_vzdalenost / self.delka_kroku)
            self.mcu_model.msg_poslat_bez_odpovedi("#S") #stop jakekoliv funkce co zrovna jede na MCU
            time.sleep(0.2)
            self.mcu_model.msg_poslat_bez_odpovedi("#K") #zapnout rezim kalibrace
            ...
            #zapis LookupTabulky do MCU
            self.mcu_model.mcu_serial.ser.reset_input_buffer()
            self.mcu_model.msg_poslat_bez_odpovedi("#S") #stop jakekoliv funkce co zrovna jede na MCU
            time.sleep(0.5)
            self.mcu_model.msg_poslat_bez_odpovedi("#L") #zapnout rezim nahrani LookupTabulky
            time.sleep(0.5)
            LT_velikost = int(self.pocet_kroku+1)
            self.mcu_model.msg_poslat_bez_odpovedi(f"#{LT_velikost}")
            time.sleep(0.5)
            data_filtrovane = []
            data_posunuti = []
            
            for i in range(self.pocet_kroku+1):
                    data_posunuti.append(int(i * self.delka_kroku))
                    print(int(i * self.delka_kroku))

                    prumer = 100
                    print(f"{i};{prumer}")
                    data_filtrovane.append(int(prumer))
                    
            for i in range(LT_velikost):
                x = int(data_posunuti[i])
                y = int(data_filtrovane[i])
                msg = f"#{x};{y}"

                self.mcu_model.msg_poslat_bez_odpovedi(msg)
                time.sleep(0.2)
                if not self.mcu_model.precist_ok(timeout=5.0):
                    print(f"[Chyba] MCU nereaguje na {msg}")
                    break  # nebo raise Exception
